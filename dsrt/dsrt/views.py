...
def visualComp(request):
	...
	for roi in roiList:
		...
		distance=[round(x,2) for x in sortedDataset[['distance'][0]]]
               #Change these lines to see the dose for test patients
		#doseMean=np.insert(doseMean,0,queryDoseMean)
		doseMean=np.insert(doseMean,0,0)
...
def getPixelData(request):
	...
	cur.execute('SELECT width, height, fk_sop_id FROM ct_image WHERE fk_patient_id="'+str(fk_patient_id)+'" ORDER BY zIndex DESC')
	result=cur.fetchall()
